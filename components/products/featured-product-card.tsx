...
export function FeaturedProductCard({ product }: FeaturedProductCardProps) {
  ...
  return (
    ...
    >
      {/* Featured Badge */}
      <div className="absolute top-6 left-6 z-20 bg-gradient-to-r from-amber-500 to-orange-500 text-white px-4 py-2 rounded-full font-semibold text-sm flex items-center gap-2 shadow-lg">
      ...
      <div className="grid md:grid-cols-2 gap-8 p-8 md:p-12">
        ...
        <div className="flex flex-col justify-center space-y-6">
          <div>
            ...
            {product.description && (
              <p className="text-gray-300 text-lg leading-relaxed line-clamp-4">{product.description}</p>
            )}
          </div>

          <div className="space-y-4">
            <div className="flex items-baseline gap-3">
              <span className="text-5xl font-light text-amber-400">£{Number.parseFloat(price.amount).toFixed(2)}</span>
              {!product.availableForSale && <span className="text-red-400 text-sm font-medium">Out of Stock</span>}
            </div>

            <div className="inline-flex items-center gap-2 bg-gradient-to-r from-orange-500 to-amber-500 text-white px-8 py-4 rounded-full font-semibold text-lg group-hover:shadow-lg group-hover:shadow-amber-500/50 transition-all duration-300">
